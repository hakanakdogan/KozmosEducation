...
namespace Application.Courses.Queries.GetInstructorCourses
{
    public class GetInstructorCoursesQuery : IRequest<Result<List<CourseDto>>>
    {
    }
    public class GetInstuctorCoursesQueryHandler : IRequestHandler<GetInstructorCoursesQuery, Result<List<CourseDto>>>{
        private readonly DataContext _dataContext;
        private readonly IUserAccessor _userAccessor;

        public GetInstuctorCoursesQueryHandler(DataContext dataContext, IUserAccessor userAccessor)
        {
            _dataContext = dataContext;
            _userAccessor = userAccessor;
        }

        public async Task<Result<List<CourseDto>>> Handle(GetInstructorCoursesQuery request, CancellationToken cancellationToken)
        {
            ...
            var userCourses = await _dataContext.Courses
                ...
                .Where(x => x.Attendees.Any(u => u.AppUserId == user.Id && u.IsCreator == true)).Select(xx => new CourseDto
                {
