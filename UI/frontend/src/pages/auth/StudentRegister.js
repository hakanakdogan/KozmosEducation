...
const StudentRegister = () => {
  ...
  const registerUser = async (values) => {
    // const response = await agent.Account.userRegister(values);
    setErrors([]);
    try {
  ...
  return (
    <AppLayout>
      <CustomBreadcrumb title="Kayıt Ol" description="Bir öğrenci hesabı oluştur ve öğrenmeye başla..." />
        ...
        {({
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mt-3 align-auth">
              <div>
                <div><small>Zaten üye misin? <Link to="/login"><b>Giriş Yap</b></Link></small></div>
                <div><small>Öğretmen misin? <Link to="/teacher/register"><b>Öğretmen Olarak Katıl</b></Link></small></div>
              </div>
              <Button
                className="green-btn"
                type="submit"
